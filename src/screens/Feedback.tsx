...
export default function Feedback({ navigation }: any) {
  ...
  const submit = async () => {
    ...
    await AsyncStorage.setItem(`mana-kandukur-feedback-${Date.now()}`, JSON.stringify({ ...form, type, createdAt: new Date().toISOString() }))
    setError('')
    setSubmitted(true)
  }
      ...
      <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
        ...
        {submitted ? (
          ...
          <View style={styles.card}>
            ...
            <Pressable style={styles.primary} onPress={submit}><Text style={styles.primaryText}>{t('Send message', 'సందేశం పంపండి')}</Text></Pressable>
          </View>
        )}
      </ScrollView>
